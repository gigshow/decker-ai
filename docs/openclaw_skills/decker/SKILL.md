...
**사용자에게 제공할 URL**: `https://decker-ai.com`, `@deckerclawbot` (t.me/deckerclawbot). 연동은 로그인 후 **설정 → 텔레그램**.
...
2. **연동** — 로그인 후 **설정 → 텔레그램**에서 연동 코드 발급 → @deckerclawbot 에 전송
   (개발자·에이전트: **설정 → API 키**에서 키 발급 → 시그널·상태·시장 읽기는 연동 없이도 가능)
...
- **읽기 (시그널·상태·시장·비교·청산)**: 이 스킬로 **지금 바로** — 연동 불필요(공개 API / API 키).
- **주문·포지션·자동주문**: **Slack 연동된 사용자**만 이 스킬로 처리(아래 order-request). **신규·텔레그램 사용자는 Decker 자체 텔레그램 에이전트 [@deckerclawbot](https://t.me/deckerclawbot)** 에서 주문(로그인→설정→텔레그램 연동, 실주문 지원·검증됨). Slack 셀프 연동 페이지는 현재 미제공.

## Goal

**읽기는 말만 하면** 시그널·상태·시장(연동 없이). **주문**은 Slack 연동 사용자면 web_fetch GET order-request 로, 아니면 **@deckerclawbot 안내**. "자동화 미지원"이라 잘라 말하지 말고 **주문 경로(@deckerclawbot)를 안내할 것**.
...
| "내 Slack ID 알려줘" | channel_user_id로 직접 응답 | Decker가 ID + 연동 안내(설정→텔레그램) |
| "채널 ID 알려줘" | channel_id로 직접 응답 | Decker가 ID + 연동 안내(설정→텔레그램) |
...
- 연동: 로그인 → 설정 → 텔레그램 → 연동 코드 발급 → @deckerclawbot
...
- 사용자에게 제공할 URL: **https://decker-ai.com**, **@deckerclawbot** (t.me/deckerclawbot) 만
...
**시작하려면**: ① https://decker-ai.com 가입 (무료) ② 로그인 → 설정 → 텔레그램 연동 → @deckerclawbot ③ 연동 후 바로 사용 가능! (시그널·상태 읽기는 설정 → API 키 발급 후 연동 없이도)
...
**반드시 아래 응답을 사용할 것.** "가입 정보 제공 불가", "공식 웹사이트 확인" 등으로 대체하지 말 것. **반드시 decker-ai.com·@deckerclawbot 포함.**
...
1. **Decker 가입** — https://decker-ai.com 에서 회원가입 (무료)
2. **연동** — 로그인 후 **설정 → 텔레그램**에서 연동 코드 발급 → @deckerclawbot 에 전송
   (시그널·상태·시장 읽기는 **설정 → API 키** 발급 후 연동 없이도 가능)
...
- 응답: "Slack ID는 `{id}`예요. 😊" / "채널 ID는 `{id}`예요. 😊" + 연동 안내(설정→텔레그램)
...
5. **연동 안 됐으면 404** → 사용자에게 친절한 가이드 전달:
   - "Decker에 가입하고 Slack을 연동하면 포지션 조회·수익현황·주문 등을 사용할 수 있어요. ① https://decker-ai.com 가입 ② 로그인 → 설정 → 텔레그램 연동 → @deckerclawbot ③ 연동 완료 후 다시 요청해 주세요! (시그널·상태 읽기는 설정 → API 키 발급 후 연동 없이도)"
...
- **404 (JWT/연동)**: 가입→연동 가이드(설정→텔레그램) 전달
...
| "가입 정보 제공 불가", "공식 웹사이트 확인" | 반드시 decker-ai.com·@deckerclawbot 제공 |
...
→ 위와 같은 응답은 **절대** 하지 말 것. 대신 3단계 가이드(decker-ai.com, @deckerclawbot)만.
